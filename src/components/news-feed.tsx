...
export function NewsFeed({ initialItems = [], mode = 'default', initialTag }: NewsFeedProps) {
    ...
    const combined = useMemo(() => {
        const baseItems = sortBy === 'popular' ? [] : feedItems;
        const allItems = [...baseItems, ...loadedItems];
    ...
    }, [feedItems, loadedItems, sortBy]);

    // Apply Batch Translation
    const translatedCombined = useBatchTranslation(combined);

    // Filter logic
    const displayItems = useMemo(() => {
        let result = translatedCombined;

        if (mode === 'focus') return result;
    ...
    const handleShare = (item: NewsItem, platform: 'copy' | 'twitter' | 'facebook') => {
        ...
        if (platform === 'copy') {
            ...
            setToast({ message: t('feed.toast.linkCopied'), type: 'success' });
                ...
                displayItems.length > 0 ? (
                    ...
                    <div className="text-center py-12 text-gray-500 bg-white/30 dark:bg-gray-900/30 rounded-xl border border-dashed border-gray-300 dark:border-gray-700">
                        {isLoading ? (
                            ...
                            ? t('feed.noResults').replace('{tag}', selectedTag)
                            : t('feed.empty')
                        }
                    </div>
                )
