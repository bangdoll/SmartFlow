...
export function BookmarkCard({ item }: BookmarkCardProps) {
    ...
    return (
        <div className="relative group">
            {/* Remove Button - OUTSIDE the Link, positioned absolutely */}
            <button
                onClick={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
                ...
                className="absolute top-4 right-4 z-20 p-2 rounded-full text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-all bg-white/80 dark:bg-black/80 backdrop-blur-sm shadow-sm opacity-0 group-hover:opacity-100"
            ...
            <Link
                href={linkHref}
                className="block bg-white/60 dark:bg-gray-900/60 backdrop-blur-sm border border-white/50 dark:border-gray-800/50 rounded-xl p-6 hover:shadow-lg hover:scale-[1.01] transition-all duration-300 shadow-sm cursor-pointer"
            >
                {/* Meta Row */}
