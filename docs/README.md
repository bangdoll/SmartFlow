...
- **自動化週報 (Weekly Trends)**: 每週一早上自動分析過去 7 天新聞，產生趨勢關鍵字與洞察報告。
- **思維模型分析 (Mental Models)**: 引入查理·芒格 (Charlie Munger) 的 100 個思維模型庫，每週隨機挑選 3 個模型（如機會成本、反熵增）進行深度新聞解讀，保持分析視角的新鮮度。
- **趨勢頁面 (Trends Page)**: `Trends` 頁面完全自動化，同步週報資料庫，並支援中英雙語切換。
- **比較頁面 (Compare Page)**: 根據真實新聞數據自動推薦熱門比較主題 (e.g., Apple vs Google)，並包含 2026/2027 未來展望。
- **自動修復與翻譯**: 每日排程自動檢查並修復雙語內容缺失，確保體驗一致。
...
- **每日自動修復增強 (2025-12-28)**: 雙語自動修復機制擴展至 14 天、每次處理 50 則；批次翻譯 205 則英文標題為繁體中文，確保中文版網站全部顯示中文。
...
| `/api/cron/scrape` | 執行爬蟲，抓取最新新聞 | 每 15 分鐘 |
