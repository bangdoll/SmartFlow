...
- **成長策略功能上線 (2025-12-24)**: 新增 `/monthly` 月報頁面（30 天趨勢統計）、`/essentials` 必讀頁面（AI 入門精選）、`/compare` 比較頁面（GPT-5 vs Gemini 3 等 SEO 頁面）；整合社群貼文模板系統，新聞詳情頁可一鍵複製 X/LinkedIn 貼文；訂閱文案改為風險導向以提升轉換率。
- **週報頁面與內容品質優化 (2025-12-23)**: 新增 `/weekly` 週報頁面，展示過去 7 天 AI 決策風險報告；行動裝置改為漢堡選單導覽；批次修復 110+ 則新聞的中文標題與摘要；解決 Favicon 不顯示、新聞詳情頁亂碼等問題。
- **導航穩定性與效能修復 (2025-12-22)**: 解決首頁「我的收藏」連結無法點擊的問題、優化書籤卡片整卡點擊體驗，並大幅提升語言切換的回應速度（從延遲數秒變為瞬間生效）。
- **體驗細節打磨 (2025-12-21)**: 優化新聞卡片點擊觸發面積、修復移動端導航列佈局，並完善全站按鈕與提示語的在地化翻譯，確保 iOS PWA 下的登入狀態穩定。
