...
- **按鈕簡化 (UI Simplification)**:
  - **深色模式**: 將「淺色/深色/系統」三按鈕簡化為單一「太陽/月亮」切換鍵，釋放 60% 空間。
  - **語言切換**: 將下拉選單簡化為單一「中/EN」切換鍵，提供更直覺的一鍵切換體驗。
- **手機版體驗 (Mobile Experience)**:
  - **首頁 Logo**: 優化左上角 Logo 的觸控範圍 (`py-2`)，確保單手操作也能輕鬆點擊回到首頁。
  - **新增歷史入口**: 在手機版導覽列新增「時鐘」圖示，讓手機用戶也能快速進入歷史新聞庫。
  - **視窗防遮擋**: 調整全站主要內容區塊 (`pt-24`)，防止內容被固定置頂的 Header 遮擋。
- **完整在地化 (Full Localization)**:
  - **趨勢頁面**: 導入 `TrendsView` 客戶端元件，實現趨勢內容的中英文即時切換。
  - **歷史頁面**: 將標題改為動態元件，支援 "News Archive" 與 "新聞存檔" 雙語顯示。

## [2025-12-16] 體驗細節打磨與 AI 深度整合

### 優化 (Optimized)
- **零閃爍切換 (Instant Hydration Fix)**: 徹底解決繁體中文內容載入時的「英文閃爍」問題，採用 `suppressHydrationWarning` 技術，確保用戶進入網站即看到中文，且點擊與互動完全正常。
- **手機版排版 (Mobile Layout)**: 
  - 優化新聞卡片底部佈局，採用分層顯示（Tags 在上，按鈕在下）。
  - 對「閱讀完整分析」等操作按鈕加上強制不換行 (`whitespace-nowrap`)，防止文字垂直排列。
- **音訊播放器 (Audio Player)**: 
  - 增強「播放速度 (1x)」按鈕的對比度（深藍底白字）與點擊範圍，大幅提升長輩的可讀性與易操作性。
- **摘要生成策略**: 嚴格限制 AI 在產生「白話解讀」時的字數（50 字以內），確保內容精簡好讀。

### 新增 (Added)
- **AI 語言同步 (AI Language Sync)**: 
  - **Chat 助手**: 切換英文介面時，AI 助手將強制以英文回答問題。
  - **語音導讀**: 切換英文介面時，系統會自動播放（或即時翻譯）英文標題與英文摘要，實現真正的雙語聽讀體驗。
- **導航升級**: 將「今日重點 (Daily Insight)」區塊升級為強力導航模式 (Hard Navigation) 並搭配 8 碼短網址，解決部分裝置無法點擊的問題。
